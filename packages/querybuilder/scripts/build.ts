...
const querybuilderDeclaration = (dialect: "postgres" | "mysql"): string => `import type * as Effect from "effect/Effect"
...
const postgresMetadataDeclaration = `export interface ColumnModel {
...
const writeDeclarations = async (): Promise<void> => {
  await mkdir(join(distDir, "postgres"), { recursive: true })
  await writeFile(join(distDir, "postgres.d.ts"), querybuilderDeclaration("postgres"))
  await writeFile(join(distDir, "mysql.d.ts"), querybuilderDeclaration("mysql"))
  await writeFile(join(distDir, "postgres/metadata.d.ts"), postgresMetadataDeclaration)
}
